...
% FIX: Adjusted column widths and font sizes to avoid overfull boxes
\begin{frame}%[fragile]
  \frametitle{Новый алгоритм}
  \framesubtitle{Иллюстративные возможности: таблицы, картинки, код}
  \begin{columns}[T]
    \begin{column}[t]{.4\textwidth}
      % Use \columnwidth for minipage width, and reduce font size for verbatim
      ...
      \vspace{2ex}
      Грамматика:
      % FIX: Use \coloneqq (from mathtools) instead of \Coloneq, and reduce font size
      {\footnotesize
      \begin{align*}
        start & \coloneqq  s                                              \\
        s     & \coloneqq  \mbox{\texttt{LBR }} s \mbox{\texttt{ RBR }} s \\
        s     & \coloneqq  \varepsilon
      \end{align*}}
    \end{column}
    \begin{column}[T]{.6\textwidth}
      \vspace{-2em}
      \includegraphics[width=\columnwidth]{figures/out3.pdf}
    \end{column}
  \end{columns}
\end{frame}

% FIX: Reduced spacing between theorems to fix overfull \vbox
\begin{frame}
...
% FIX: Use \textwidth instead of fixed 13cm to avoid overfull hbox
\begin{frame}[t]
